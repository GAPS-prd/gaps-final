...
export default function NotesPage() {
  ...
  useEffect(() => {
    const fetchNotes = async () => {
      const notesCollection = collection(db, "journals");
  ...
  const createNote = async () => {
    try {
      ...
      const newNote = {
        userId,
        title: "New Journal", // Default title, consider prompting the user for input
      ...
      const docRef = await addDoc(collection(db, "journals"), newNote);

      // Redirect to the newly created note's page
      router.push(`/journal/${docRef.id}`);
    } catch (error) {
  ...
  return (
    <div className="w-full p-4">
      <div className="flex w-full justify-between items-center">
        <h1 className="font-medium text-3xl">Your Journals</h1>
      ...
      <ul className="space-y-1">
        {notes.map(note => (
          <li key={note.id} className="text-blue-600 font-medium text-lg hover:underline">
            <Link href={`/journal/${note.id}`}>{note.title}</Link>
          </li>
        ))}
      </ul>
    </div>
